$(document).ready(function(){
	var authToken = "a21877a1d0c15d20e9bcbe054a2fbc67453f548d";
	var machines = [];
	var users = [];
    $.ajax
	  ({
	...
	$(document).on("click","#machine",function(){
		...
		for(x in req_mac["last_logged_in_users"]){
			$("#well").append("<p data-id="+req_mac['last_logged_in_users'][x]+" id='user'>      "+req_mac['last_logged_in_users'][x]+"</p>")
		}
	...
	$(document).on("click","#user",function(){
		...
		for(x in req_mac["login_sessions"]){
			$("#well").append("<p data-id="+req_mac['login_sessions'][x]+" id='machine'>      "+req_mac['login_sessions'][x]+"</p>")
		}
	});
});
